...
z1, z2, c2 = 31, 37, 1733  # Parameters for double hashing
initial_table_size = 2053  # Initial table size
...
# Print Load Factors
def print_load_factors():
    print(f"HashMap (Chain) Load Factor: {hashmap_chain.get_load():.2f}")
    print(f"HashMap (Linear) Load Factor: {hashmap_linear.get_load():.2f}")
    print(f"HashMap (Double) Load Factor: {hashmap_double.get_load():.2f}")
    print(f"HashSet (Chain) Load Factor: {hashset_chain.get_load():.2f}")
    print(f"HashSet (Linear) Load Factor: {hashset_linear.get_load():.2f}")
    print(f"HashSet (Double) Load Factor: {hashset_double.get_load():.2f}")
...
end_time = time.time()
print(f"Time for HashMap : {end_time - start_time:.2f} seconds")
